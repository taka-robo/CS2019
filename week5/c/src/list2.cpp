...
struct node *insert_after(struct point point, struct node *pt) //挿入したいkeyの値と次に挿入したい場所のポイン
{
    ...
    new_node->cell.step = point.step; //新しいノードのstepにstepの値を格納
...
void print_whole_list(struct node *pt)
{
    ...
    {
        printf("%d,%d(%d) -> ", pt->cell.col, pt->cell.row, pt->cell.step); //pのkeyを表示して
    }
    printf("\n");
}
/**
...
{
    std::cout << __func__ << ":" << col << "," << row << "(" << step << ")"
              << "   ";
    struct node *p;
    struct point pt;
    pt.col = col;
...
struct point pop(struct node *head)
{
    ...
    //pの次の要素の値を取得し削除
    temp = delete_next(p);
    std::cout << ":" << temp.col << "," << temp.row << "(" << temp.step << ")"
              << "   ";
    print_whole_list(head);
    return temp;
}
